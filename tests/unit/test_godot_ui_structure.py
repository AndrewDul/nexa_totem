...
class GodotUiStructureTests(unittest.TestCase):
    ...
    def test_hardware_gateway_foundation_is_represented(self):
        ...
        self.assertIn("presence_show_clock_after_seconds := 3.0", main)
        self.assertIn("presence_show_face_after_seconds := 0.5", main)
        self.assertIn("presence_clock_active_from_hardware", main)
        self.assertIn("func _is_distance_valid_for_presence", main)
        self.assertIn("func _update_presence_face_clock", main)
        self.assertIn("joystick_repeat_delay_seconds := 0.28", main)
        self.assertIn("joystick_select_cooldown_seconds := 0.55", main)
        self.assertIn("joystick_select_armed", main)
        self.assertIn("func _dispatch_nexa_input_action", main)
        self.assertIn("func _handle_hardware_joystick", main)

    def test_esp_pull_mode_and_wifi_switch_foundation_are_represented(self):
        root = REPO_ROOT
        live_api = self.read(root / "system/services/diagnostics/live_api.py")
        esp_pull_client = self.read(root / "system/services/hardware_gateway/esp_pull_client.py")
        wifi_plan = self.read(root / "system/network/wifi_switch/wifi_switch_plan.py")
        connect_script = self.read(root / "scripts/network/connect_to_esp_network.py")
        reconnect_script = self.read(root / "scripts/network/reconnect_home_wifi.py")
        esp_mode_launcher = self.read(root / "scripts/run/run_nexa_totem_esp_mode.sh")
        normal_launcher = self.read(root / "scripts/run/run_godot_ui_with_api_dev.sh")
        esp_readme = self.read(root / "hardware/esp8266/README.md")
        architecture = self.read(root / "docs/architecture.md")
        troubleshooting = self.read(root / "docs/troubleshooting.md")

        self.assertTrue((root / "system/services/hardware_gateway/esp_pull_client.py").exists())
        self.assertTrue((root / "scripts/run/run_esp_pull_client_dev.py").exists())
        self.assertTrue((root / "system/network/wifi_switch").exists())
        self.assertTrue((root / "system/network/wifi_switch/README.md").exists())
        self.assertTrue((root / "scripts/network/connect_to_esp_network.py").exists())
        self.assertTrue((root / "scripts/network/reconnect_home_wifi.py").exists())
        self.assertTrue((root / "scripts/run/run_nexa_totem_esp_mode.sh").exists())
        self.assertIn("class EspPullClient", esp_pull_client)
        self.assertIn("http://192.168.4.1/api/state", esp_pull_client)
        self.assertIn("urllib.request", esp_pull_client)
        self.assertIn("fetch_and_update", esp_pull_client)
        self.assertIn("NEXA_HARDWARE_MODE", live_api)
        self.assertIn("pull_esp_server", live_api)
        self.assertIn("/api/hardware/pull-once", live_api)
        for endpoint in ["/api/hardware/state", "/api/hardware", "/hardware-dashboard"]:
            self.assertIn(endpoint, live_api)
        for term in ["NeXa-ESP", "nexa12345", "192.168.4.1", "/api/state", "previous_wifi_connection.json"]:
            self.assertIn(term, wifi_plan)
        self.assertIn("dry_run", connect_script)
        self.assertIn("--apply", connect_script)
        self.assertIn("--i-understand-this-will-disconnect-internet", connect_script)
        self.assertIn("changed_network", connect_script)
        self.assertIn("dry_run", reconnect_script)
        self.assertIn("--apply", reconnect_script)
        self.assertIn("--i-understand-this-changes-network", reconnect_script)
        self.assertIn("--delete-esp-profile", reconnect_script)
        self.assertIn("CONFIRM_NEXA_ESP_WIFI_SWITCH", esp_mode_launcher)
        self.assertIn("ESP mode will disconnect Raspberry Pi from normal Wi-Fi/internet and connect to NeXa-ESP.", esp_mode_launcher)
        self.assertIn("connect_to_esp_network.py --apply --i-understand-this-will-disconnect-internet", esp_mode_launcher)
        self.assertIn("export NEXA_HARDWARE_MODE=pull_esp_server", esp_mode_launcher)
        self.assertIn("export NEXA_ESP_STATE_URL=http://192.168.4.1/api/state", esp_mode_launcher)
        self.assertIn("export NEXA_ESP_POLL_INTERVAL_SECONDS=0.2", esp_mode_launcher)
        self.assertIn("run_godot_ui_with_api_dev.sh", esp_mode_launcher)
        self.assertIn("trap cleanup EXIT INT TERM", esp_mode_launcher)
        self.assertIn("reconnect_home_wifi.py --apply --i-understand-this-changes-network", esp_mode_launcher)
        self.assertNotIn("connect_to_esp_network.py", normal_launcher)
        self.assertNotIn("reconnect_home_wifi.py", normal_launcher)
        self.assertIn("ESP Server Mode", esp_readme)
        self.assertIn("Push Mode", esp_readme)
        self.assertIn("NeXa-ESP", architecture)
        self.assertIn("Raspberry Pi can connect to `NeXa-ESP`", architecture)
        self.assertIn("Real switching is manual/apply-gated", architecture)
        self.assertIn("ESP Server Mode disconnects internet", troubleshooting)
